...
MATLAB/Psychtoolbox utilities for validating the Holt Laboratory experimental hardware configuration before running behavioral and EEG experiments.

These scripts test individual components and hardware pathways involving Brain Products, VPixx Technologies, and RME devices. They are intended to verify hardware communication, trigger transmission, display timing, audiovisual synchronization, and behavioral response acquisition before validating or running a complete experimental pipeline.
...
For quantitative validation and calibration of auditory stimulus timing, see the [`Audio_Timing`](../Audio_Timing/) tools.